        ...
        with st.sidebar:
            ...
            with col1:
                if st.button("🧪 Test OpenAI", use_container_width=True):
                    with st.spinner("Testing OpenAI connection..."):
                        from test_functions import test_openai_connection
            ...
            with col2:
                if st.button("🧪 Test Groq", use_container_width=True):
                    with st.spinner("Testing Groq connection..."):
                        from test_functions import test_groq_connection
            ...
            if st.button("🔍 Test Embeddings", use_container_width=True):
                with st.spinner("Testing embeddings connection..."):
                    from test_functions import test_embeddings_connection
                    result = test_embeddings_connection(self.config_path)
                    if result["success"]:
                        st.success(f"✅ {result['message']}")
                    else:
                        st.error(f"❌ {result['error']}")
            ...
            if st.button("⚡ Test All APIs", use_container_width=True):
                ...
                # Test OpenAI
                from test_functions import test_openai_connection, test_groq_connection, test_embeddings_connection
                ...
                # Test Embeddings
                embed_result = test_embeddings_connection(self.config_path)
